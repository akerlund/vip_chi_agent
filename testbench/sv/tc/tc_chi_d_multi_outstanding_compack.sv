class tc_chi_d_multi_outstanding_compack extends vip_chi_base_test;

  `uvm_component_utils(tc_chi_d_multi_outstanding_compack)
  ...
  // ---------------------------------------------------------------------------
  // Multi-outstanding on the RN-I and buffered SN-F. Writes set ExpCompAck, so
  // each carries an NCBWrDataCompAck burst and the RN-I must drive a CompAck back
  // after completion -- the SN-F otherwise times out waiting for it.
  // ---------------------------------------------------------------------------
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Pipeline N ExpCompAck writes, then read them back. The SN-F waits for a
  // matching CompAck per write (txn_id + src/tgt); if the pipeline failed to
  // drive it, the SN-F would fatal on timeout. Passing therefore proves the
  // CompAck handshake overlapped correctly, and the read-back proves the data
  // landed.
  // ---------------------------------------------------------------------------
  // ---------------------------------------------------------------------------
  // Run Phase
  ...
  task run_phase(input uvm_phase phase);
    ...
    // -- Phase 1: pipeline N ExpCompAck writes. --------------------------------
    ...
    foreach (wr_rsp[k]) begin
      ...
      if (w.rsp_opcode != item_t::rsp_opcode_t'(VIP_CHI_RSP_COMP_DBID_RESP_C)) begin
        `uvm_fatal(get_name(), $sformatf(
          "FATAL [%s] ExpCompAck write %0d carried RSP opcode 0x%0h, expected CompDBIDResp",
    ...
    if (super.rni_cfg.observed_peak_outstanding <= 1) begin
      `uvm_fatal(get_name(), $sformatf(
        "FATAL [%s] ExpCompAck writes did not overlap: peak in-flight was %0d (expected > 1)",
        super.tc_name, super.rni_cfg.observed_peak_outstanding))
    end

    `uvm_info(get_name(), $sformatf(
      "INFO [%s] %0d ExpCompAck writes pipelined (CompAck accepted); read-back verified; peak in-flight = %0d",
