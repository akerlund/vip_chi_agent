class tc_chi_d_multi_outstanding_split extends vip_chi_base_test;

  `uvm_component_utils(tc_chi_d_multi_outstanding_split)

  localparam int            N_C         = 6;
  localparam item_t::addr_t BASE_ADDR_C = item_t::addr_t'(44'h2B00_0000);
  ...
  // ---------------------------------------------------------------------------
  // Multi-outstanding on the RN-I and buffered SN-F, plus the SN-F's split write
  // policy: each write is granted a DBIDResp and completed by a later deferred
  // Comp instead of a combined CompDBIDResp. The pipeline must overlap these
  // two-flit completions correctly.
  // ---------------------------------------------------------------------------
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Pipeline N split writes, confirm each hands back a deferred Comp (not a
  // combined CompDBIDResp), then read the addresses back and check every write
  // landed. Overlap is confirmed via the peak in-flight count.
  // ---------------------------------------------------------------------------
  // ---------------------------------------------------------------------------
  // Run Phase
  // ---------------------------------------------------------------------------
  task run_phase(input uvm_phase phase);
    ...
    // -- Phase 1: pipeline N split writes. -------------------------------------
    ...
    foreach (wr_rsp[k]) begin
      ...
      // Split policy: the handed-back completion is a deferred Comp, not the
      // combined CompDBIDResp that the default policy would return.
      if (w.rsp_opcode != item_t::rsp_opcode_t'(VIP_CHI_RSP_COMP_C)) begin
        `uvm_fatal(get_name(), $sformatf(
          "FATAL [%s] Split write %0d carried RSP opcode 0x%0h, expected Comp",
    ...
    if (super.rni_cfg.observed_peak_outstanding <= 1) begin
      `uvm_fatal(get_name(), $sformatf(
        "FATAL [%s] Split writes did not overlap: peak in-flight was %0d (expected > 1)",
        super.tc_name, super.rni_cfg.observed_peak_outstanding))
    end

    `uvm_info(get_name(), $sformatf(
      "INFO [%s] %0d split (DBIDResp+Comp) writes pipelined; read-back verified; peak in-flight = %0d",
