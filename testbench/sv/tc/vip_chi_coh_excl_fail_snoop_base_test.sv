// ===========================================================================
// vip_chi_coh_excl_fail_snoop_base_test
//
// Exclusive store FAILS because a snoop-invalidate cleared the monitor (clear
// path (b): the home's own SnpUnique to the reserving port). RN-F0 takes an
// exclusive load, then RN-F1 ReadUniques the line -- NOT a store, but it forces
// the home to SnpUnique(RN-F0), and that invalidating snoop clears RN-F0's
// per-port monitor bit. RN-F0's later exclusive store must therefore LOSE:
//   RN-F0 LL (ReadClean+excl) L  -> SC, monitor[L][0]=1, Comp/ExclOkay
//   RN-F1 ReadUnique L           -> SnpUnique(RN-F0) -> monitor[L][0] cleared
//   RN-F0 SC (CleanUnique+excl) L -> monitor clear -> Comp/NormalOkay (FAIL)
// This isolates the snoop-invalidate clear path from the remote-store clear path
// exercised by tc_chi_coh_d_excl_fail_store (there RN-F1 issues a genuine store).
// Asserts: LL won ExclOkay, SC reports NormalOkay (lost), RN-F0 cache stays I,
// Checker D silent.
//
// The scenario body is this parameterized base test; the runnable subclasses
// (own files) run it at CHI-D (tc_chi_coh_d_excl_fail_snoop) and wide CHI-E
// (tc_chi_coh_e_excl_fail_snoop).
//
// Used by:
//   tc_chi_coh_e_excl_fail_snoop  (wide CHI-E)
//   tc_chi_coh_d_excl_fail_snoop    (CHI-D)
// ===========================================================================
class vip_chi_coh_excl_fail_snoop_base_test #(
  ...
  `uvm_component_param_utils(vip_chi_coh_excl_fail_snoop_base_test #(CFG_P, TYPES_T))
  ...
  task run_phase(input uvm_phase phase);
    ...
    // RN-F1 ReadUnique -> home SnpUniques RN-F0, clearing its monitor bit.
    this.cfg_read_seq(super.hrnf1_rdunique_seq);
    super.hrnf1_rdunique_seq.start(super.tb_env.hrnf1_agent.sequencer);
    ru_rsp = super.hrnf1_rdunique_seq.get_responses();
    ...
    if ((ll_rsp.size() != 1) || (ru_rsp.size() != 1) || (sc_rsp.size() != 1)) begin
      `uvm_fatal(get_name(), $sformatf(
        "FATAL [%s] expected 1+1+1 responses, got %0d/%0d/%0d",
        super.tc_name, ll_rsp.size(), ru_rsp.size(), sc_rsp.size()))
    end
    ...
    // The store lost: the SnpUnique from RN-F1's ReadUnique cleared the monitor.
    ...
    // RN-F0 was snoop-invalidated; the lost SC must not re-take the line.
    ...
    if (rnf0_state != VIP_CHI_RESP_STATE_I_E) begin
      `uvm_fatal(get_name(), $sformatf(
        "FATAL [%s] RN-F0 cache 0x%0h after a LOST SC, expected I",
