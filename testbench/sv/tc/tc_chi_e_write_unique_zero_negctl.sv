...
class tc_chi_e_write_unique_zero_negctl extends chi_e_base_test;
  ...
  //
  // The raw-inject path scopes a REQ's outstanding window to the flit itself --
  // "a raw flit is a single injected packet with no completion to wait for" --
  // which was sound while every raw-injectable REQ opcode had no modeled
  // completion. It is not sound for this one: the checker tracks the request as
  // outstanding until its completion arrives, so the sideband would go low with a
  // transaction still live and CHI_TXSACTIVE_COVERS_OUTSTANDING would report it,
  // correctly and about the wrong thing.
  //
  ...
  // exists to model a node that speculates on more traffic. 32 cycles covers the
  // widest injection-to-completion gap below with room to spare; the deassert
  // bound moves with it, because the checker reads the same value.
