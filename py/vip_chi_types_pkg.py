...
class SnpOpcode(IntEnum):
  ...
  # of a line without disturbing it -- see snp_opcode_preserves_state.
  QUERY = 0x10
  SHARED_FWD = 0x11
  CLEAN_FWD = 0x12
  ONCE_FWD = 0x13
...
# drop". SnpMakeInvalidStash belongs in this set when it is modeled.
#
# SnpQuery is in it for an unrelated reason that reaches the same place. 4.5:
# "Snoopee must not return data with the Snoop response." It is not discarding a
# dirty copy the way SnpMakeInvalid is -- it keeps the copy, and keeps the state
# that says so -- but the channel rule is identical, so check_snp_resp_form
# judges both with one predicate.
SNP_NO_DATA_OPCODES = frozenset({
  int(SnpOpcode.MAKE_INVALID), int(SnpOpcode.QUERY),
})
...
# Every snoop opcode that appears anywhere in Table 4-5, taken as the union of
# the table above rather than written out again. One table, two questions: an
# opcode added to a row is in this set the same moment, and cannot be in one and
# not the other.
_TABLE_4_5_GENERATED_SNOOPS_C = frozenset(
  op for ops in _SNOOPS_PERMITTED_FOR_REQ.values() for op in ops)


def snp_opcode_generated_by_request(snp_op: int) -> bool:
  """True when Table 4-5 lists the opcode as generated by SOME request.
  ...
  """
  return int(snp_op) in _TABLE_4_5_GENERATED_SNOOPS_C
...
# The Forward type snoops, listed rather than derived from the encoding.
#
# Bit[4] separates them in the range Table 13-17 assigned first -- every Forward
# opcode is its non-forward opcode with that bit set, SnpUnique 0x07 ->
# SnpUniqueFwd 0x17 -- and reading the bit was how this question used to be
# answered. Issue E ends that: 0x10 SnpQuery and 0x15 SnpPreferUnique both carry
# bit[4] and neither forwards anything.
#
...
# not, so a misfiled opcode does not fail -- it takes the Fwd branch. A SnpQuery
# would have been expected to CARRY FwdNID/FwdTxnID, which Table A-6 requires it
# to zero, and the RN-F would have answered it down the direct-cache-transfer
# path, forwarding the data section 4.5 forbids it to return. Both from an
# opcode nobody had to touch this function to add.
_SNP_FWD_OPCODES_C = frozenset({
...
def snp_opcode_is_forwarding(opcode: int) -> bool:
  ...
  """
  return int(opcode) in _SNP_FWD_OPCODES_C


def snp_opcode_preserves_state(opcode: int) -> bool:
  """Whether the snoop is forbidden from changing the snoopee's cache state.

  IHI 0050 E 4.5, SnpQuery: "The SnpQuery snoop must not change the state of the
  cache line at the Snoopee." Table 4-26 says it again row by row -- all seven
  initial states have themselves as the expected final state and no "others
  ...
  """
  return int(opcode) == int(SnpOpcode.QUERY)


# Tables 4-9 and 4-10, the Resp encodings a snoop response WITHOUT data may
# carry -- 4-9 for the ordinary snoops, 4-10 for the Forward ones, and the two
# agree on this column. Written as a map from the state the snoopee holds to the
# encoding it reports.
#
# It is not the identity, and the two places it is not are the point:
#
#   * UD reports 0b010, the SAME encoding as UC. Table 4-9 gives both that row.
#     A data-less response cannot say "unique and dirty", so a Home learns that
#     the line is Unique and not whether it is modified.
...
#
# Nothing needed this before. The reduction below keeps a snoopee out of SD, and
# every other snoop moves a dirty holder either to a clean state or onto the DAT
# channel, so the map was the identity over the three states that could reach a
# data-less SnpResp. SnpQuery is the first snoop that leaves a dirty holder
# dirty and still answers on RSP.
_SNP_RESP_DATALESS_STATE_C = {
  int(Resp.I): int(Resp.I),
  int(Resp.SC): int(Resp.SC),
  int(Resp.UC): int(Resp.UC),
  int(Resp.UD_PD): int(Resp.UC),
  int(Resp.SD_PD): int(Resp.RESERVED_0),
}


def snp_resp_dataless_state(state: int) -> int:
  """The Table 4-9 / 4-10 encoding a snoopee holding `state` reports w/o data.

  Raises on a state Table 4-9 has no row for. The alternative -- returning the
  ...
  """
  s = int(state)
  if s not in _SNP_RESP_DATALESS_STATE_C:
    raise KeyError(
      f"Table 4-9 has no data-less snoop response for state 0x{s:x}")
  return _SNP_RESP_DATALESS_STATE_C[s]
...
def snp_ret_to_src_must_be_zero(opcode: int) -> bool:
  ...
  of its rows. The tables win: RetToSrc asks the snoopee to return the data, and
  4.5 forbids SnpQuery to return any.

  The "0 a" is also why SnpQuery is not simply appended to the quoted list. The
  five opcodes 4.9 names are marked plain "0" in A-6 -- applicable, and required
  to be zero. Both end as a zero on the wire, so one predicate answers for both,
  but they are not the same statement about the field.
  """
  return int(opcode) in (
    ...
    int(SnpOpcode.UNIQUE_FWD), int(SnpOpcode.QUERY),
  )
...
def snp_do_not_go_to_sd_required(issue: int, opcode: int) -> bool:
  ...
  SnpNotSharedDirty and the stash forms have no encoding here, so naming them
  would assert a reading no traffic in this tree can confirm.

  SnpQuery is here now that it has one, and its row is a "1" in Table A-6 like
  the other five. The bit reads oddly on a snoop that must not change the state
  at all -- a snoopee forbidden to transition to SD is already forbidden to
  transition anywhere -- but the table is unambiguous, and a wire value is not
  the place to apply an inference.
  """
